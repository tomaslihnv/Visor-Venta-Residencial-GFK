...
document.addEventListener('mpchange', async () => {
  ...
  if (tab === 'proyectos')    await window._pi?.renderProyectos(state, mp);
});
...
$('#resetBtn')?.addEventListener('click', async () => {
  ...
  const onChange = async _state => {
    ...
    if (tab === 'proyectos')    await window._pi?.renderProyectos(_state, mpCurrent);
...
$$('.tab').forEach(tab => {
  tab.addEventListener('click', async () => {
    ...
    switch (tab.dataset.tab) {
      ...
      case 'proyectos':    await window._pi?.renderProyectos(state, mp); break;
    }
  });
});
...
import('./miProyecto.js').then(({ getTiposState, applyTiposState }) => {
  initTiposIO({
    visorId:    'parquesindustriales',
    visorLabel: 'ParquesIndustriales',
...
// ── Consulta Inciti por área y por comuna ─────────────────────────────────
{
  ...
  async function _runQuery(polygonInciti) {
    ...
    try {
      ...
      console.error('[Inciti/parquesindustriales] Error:', err);
  ...
  function _buildPreviewPanel(entities) {
    ...
    if (txtCount) txtCount.textContent = `Inciti encontró ${entities.length} parque${entities.length !== 1 ? 's' : ''} industrial${entities.length !== 1 ? 'es' : ''}`;
    ...
    if (btnLoad) {
      btnLoad.onclick = () => {
        ...
        if (!selected.length) { alert('Debes seleccionar al menos un parque para cargar.'); return; }
        const rows = flattenEntities(selected);
        if (!rows.length) {
          console.warn('[Inciti/parquesindustriales] flattenEntities devolvió 0 filas. Entidades:', selected);
          alert('Los parques seleccionados no tienen datos disponibles. Revisa la consola del navegador.');
          return;
        }
        const fileNameEl = document.getElementById('fileName');
        if (fileNameEl) fileNameEl.textContent = `Inciti · ${selected.length} parques (${rows.length} registros)`;
  ...
  comunaBtnQuery?.addEventListener('click', async () => {
    ...
    try {
      ...
      console.error('[ParquesIndustriales/Comuna] Error:', err);
      _setStatus(`Error: ${err.message}`, 'error');
