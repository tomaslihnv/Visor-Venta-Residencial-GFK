...
function _surfaceM2(entity) {
  const chars = entity.characteristics ?? {};
  if (chars.usableM2  != null) return _num(chars.usableM2);
  if (chars.totalM2   != null) return _num(chars.totalM2);
  if (chars.gla       != null) return _num(chars.gla);
  const s = entity.surfaces;
  if (s == null) return null;
  if (typeof s === 'number') return _num(s);
  return _num(s.total ?? s.area ?? s.usable ?? s.totalM2);
}
...
export function flattenEntities(entities) {
  return entities.flatMap(entity => {
    ...
    const period = entity.periods.slice().reverse()
      .find(p => (p.avgPrice ?? 0) > 0 || (p.finalPrice ?? 0) > 0 || (p.vacancyPercent ?? 0) >= 0)
      ?? entity.periods[entity.periods.length - 1];

    const lat = loc.lat ?? null, lng = loc.lng ?? null;

    // Precio UF/m² puede venir de entity.prices o period
    const entityPrices = entity.prices ?? {};
    const ufm2 = _num(period.priceUfPerM2 ?? entityPrices.ufPerM2 ?? entityPrices.uf);
    ...
    const row = {
      ...
      'USD/m²':          _num(period.avgPrice   ?? period.finalPrice),
      'Precio Final (USD)': _num(period.finalPrice ?? period.avgPrice),
      'UF/m²':           ufm2,
      'Superficie (m²)': _surfaceM2(entity),
      'Lotes':           _num(entity.lots?.count ?? (typeof entity.lots === 'number' ? entity.lots : null)),
    ...
    return [row];
  }).filter(r => (r['Superficie (m²)'] ?? 0) > 0 || r['Vacancia (%)'] != null || (r['USD/m²'] ?? 0) > 0);
}
...
export async function queryArea({ polygons, onProgress } = {}) {
  ...
  for (const cell of cells) {
    ...
    try { payload = await _fetchPolygon(cell); }
      ...
      console.warn('[Inciti/parquesindustriales] Error en sub-zona:', err.message);
      continue;
    }
  ...
  if (!seen.size) throw new Error('El área seleccionada no contiene parques industriales en Inciti.');
