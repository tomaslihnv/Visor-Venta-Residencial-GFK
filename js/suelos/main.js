...
document.addEventListener('mpchange', async () => {
  ...
  if (tab === 'proyectos')    await window._su?.renderProyectos(state, mp);
});
...
$('#resetBtn')?.addEventListener('click', async () => {
  ...
  const onChange = async _state => {
    ...
    if (tab === 'proyectos')    await window._su?.renderProyectos(_state, mpCurrent);
...
$$('.tab').forEach(tab => {
  tab.addEventListener('click', async () => {
    ...
    switch (tab.dataset.tab) {
      ...
      case 'proyectos':    await window._su?.renderProyectos(state, mp); break;
    }
  });
});
...
// ── Consulta Inciti por área y por comuna ─────────────────────────────────
{
  ...
  async function _runQuery(polygonInciti) {
    ...
    try {
      ...
      console.error('[Inciti/suelos] Error:', err);
  ...
  function _buildPreviewPanel(entities) {
    ...
    if (txtCount) txtCount.textContent = `Inciti encontró ${entities.length} suelo${entities.length !== 1 ? 's' : ''}`;
    ...
    if (btnLoad) {
      btnLoad.onclick = () => {
        ...
        if (!selected.length) { alert('Debes seleccionar al menos un suelo para cargar.'); return; }
        const rows = flattenEntities(selected);
        if (!rows.length) {
          console.warn('[Inciti/suelos] flattenEntities devolvió 0 filas. Entidades:', selected);
          alert('Los suelos seleccionados no tienen datos disponibles. Revisa la consola del navegador.');
          return;
        }
        const fileNameEl = document.getElementById('fileName');
        if (fileNameEl) fileNameEl.textContent = `Inciti · ${selected.length} suelos (${rows.length} registros)`;
  ...
  comunaBtnQuery?.addEventListener('click', async () => {
    ...
    try {
      ...
      console.error('[Suelos/Comuna] Error:', err);
      _setStatus(`Error: ${err.message}`, 'error');
