...
document.addEventListener('mpchange', () => {
  ...
  if (activeTab === 'comparativa') {
    ...
    import('./chart.js').then(({ renderSupVsRenta }) => renderSupVsRenta());
...
$('#exportCsvBtn').addEventListener('click', () => {
  import('./data.js').then(({ state }) => {
    ...
    a.href = url; a.download = `renta_filtrada_${Date.now()}.csv`;
    a.click();
...
// ── Consulta Inciti por área / comuna ────────────────────────────────────────
{
  ...
  async function _runQuery(polygonInciti) {
    ...
    try {
      ...
      console.error('[Inciti/renta] Error:', err);
  ...
  function _buildPreviewPanel(entities) {
    ...
    if (txtCount) txtCount.textContent = `Inciti encontró ${entities.length} proyectos de renta residencial`;
    ...
    if (btnLoad) {
      btnLoad.onclick = () => {
        ...
        if (!rows.length) {
          console.warn('[Inciti/renta] flattenEntities devolvió 0 filas. Entidades:', selected);
          alert('Los proyectos seleccionados no tienen datos de arriendo disponibles. Revisa la consola del navegador.');
          return;
        }
        const allowedNames = new Set(selected.map(e => e.name).filter(Boolean));
        if (state && state.raw && state.raw.length > 0) {
          state.raw = state.raw.filter(r => allowedNames.has(r['Proyecto']));
  ...
  comunaBtnQuery?.addEventListener('click', async () => {
    ...
    try {
      ...
      console.error('[Comuna/renta] Error:', err);
      _setStatus(`Error: ${err.message}`, 'error');
