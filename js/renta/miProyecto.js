...
function _renderTipos() {
  ...
  for (const tipo of mp.tipologias) {
    ...
    // Opciones adaptadas, puedes agregar 'Studio' si lo manejan
    const typeOptions = ['Studio', '1D', '2D', '3D', '4D'].map(opt =>
...
export function initMpPanel() {
  ...
  $('#mpAddTipo')?.addEventListener('click', () => {
    mp.tipologias.push({ id: Date.now(), nombre: '', sup: null, renta: null, ufm2: null });
    _renderTipos();
    _save();
  });
