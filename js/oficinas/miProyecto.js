...
const STORAGE_KEY = 'visor_mp_of_v1';
...
export const mp = {
  ...
  util:       null,   // Útil m²
  ufm2:       null,   // UF/m²
  precio:     null,   // Precio UF
  tipologias: [],     // síntesis automática para compatibilidad con módulos core
  inMapa:     false,
  inSvp:      false,
  inDistrib:  false,
};
...
// Mantiene tipologias sincronizado con los campos planos para que los módulos
// core (SVP, Distribución, Mapa) puedan leer mp.tipologias sin distinción.
...
// Oficinas es un visor "flat" (una sola métrica por proyecto, no tarjetas de
// tipología). Exportamos/importamos igual mp.tipologias (síntesis automática)
// para reusar el mismo formato de archivo que el resto de los visores.
...
export function applyTiposState(tipologias) {
  ...
  mp.util   = t?.sup   ?? null;
  mp.ufm2   = t?.ufm2  ?? null;
  mp.precio = t?.renta ?? null;

  const set = (id, val) => { const el = document.getElementById(id); if (el) el.value = val ?? ''; };
  set('mpUtil',   mp.util);
  set('mpUfm2',   mp.ufm2);
  set('mpPrecio', mp.precio);
