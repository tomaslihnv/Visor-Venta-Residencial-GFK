...
// ── Consulta Inciti por área / comuna ────────────────────────────────────────
{
  ...
  function _buildPreviewPanel(entities) {
    ...
    if (txtCount) txtCount.textContent = `Inciti encontró ${entities.length} proyectos de oficinas`;
    ...
    const owners   = [...new Set(entities.map(e => e.owner || e.developer || ''))].filter(Boolean).sort();
    ...
    function _renderTableRows() {
      ...
      const filtered = entities.filter(e => {
        ...
        const cOwn  = !ownVal  || (e.owner || e.developer || '') === ownVal;
      ...
      tbody.innerHTML = filtered.map(e => {
        ...
        return `<tr>
          ...
          <td style="padding:8px 12px;">${e.owner || e.developer || '—'}</td>
        </tr>`;
      }).join('');
    ...
    if (btnLoad) {
      btnLoad.onclick = () => {
        ...
        if (!rows.length) {
          ...
          alert('Los proyectos seleccionados no tienen datos de precio disponibles. Revisa la consola del navegador.');
          return;
        }
        const allowedNames = new Set(selected.map(e => e.name).filter(Boolean));
        if (state && state.raw && state.raw.length > 0) {
          state.raw = state.raw.filter(r => allowedNames.has(r['Nombre']));
  ...
  comunaBtnQuery?.addEventListener('click', async () => {
    ...
    try {
      ...
      console.error('[Comuna/oficinas] Error:', err);
      _setStatus(`Error: ${err.message}`, 'error');
