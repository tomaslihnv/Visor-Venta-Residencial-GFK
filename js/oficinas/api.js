...
export function flattenEntities(entities) {
  return entities.flatMap(entity => {
    ...
    // Último periodo con precio, o simplemente el más reciente
    const period = entity.periods.slice().reverse().find(p => (p.priceUfPerM2 ?? 0) > 0)
      ?? entity.periods[entity.periods.length - 1];

    const lat = loc.lat ?? null, lng = loc.lng ?? null;
    const vac = _num(period.vacancy);
    ...
    const row = {
      ...
      'UF/m² Habilitada':  _num(period.priceHabilitada),
      'UF/m² Shell':       _num(period.priceShell),
      'Útil (m²)':         _num(chars.usableM2),
      'Vacancia (%)':      vac != null ? Math.round(vac * 100 * 10) / 10 : null,
